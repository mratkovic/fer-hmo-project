...
public class InstanceFunctionPermutation implements IFunction {
    ...
    public InstanceFunctionPermutation(final Instance problem, final FitnessCalculator calc,
            final ConstraintsChecker checker) {
    ...
    public InstanceFunctionPermutation(final Instance problem, final FitnessCalculator calc,
            final ConstraintsChecker checker, final boolean verbose) {
        super();
    ...
    private Solution findSolution(final int[] values) {
        ...
        ArrayList<Float> cpuLeft = new ArrayList<>();
        ArrayList<Float> memLeft = new ArrayList<>();
        for (int node : problem.usedNodes) {
            for (int server : problem.onNode.get(node)) {
                servers.add(server);
                cpuLeft.add(problem.cpuAvailable.get(server));
                memLeft.add(problem.memAvailable.get(server));
            }
        }

        for (int value : values) {
            // find server for that node;
            int component = problem.usedComponents.get(value);

            float cpuNeed = problem.cpuDemands.get(component);
            float memNeed = problem.memDemands.get(component);

            for (int serverId = 0; serverId < servers.size(); ++serverId) {

                float cpuHas = cpuLeft.get(serverId);
                float memHas = memLeft.get(serverId);

                if (cpuHas >= cpuNeed && memHas >= memNeed) {

                    cpuLeft.set(serverId, cpuHas - cpuNeed);
                    memLeft.set(serverId, memHas - memNeed);
                    sol.componentLocation.put(component, servers.get(serverId));
                    if (cpuHas - cpuNeed == 0 || memHas - memNeed == 0) {
                        cpuLeft.remove(serverId);
                        memLeft.remove(serverId);
